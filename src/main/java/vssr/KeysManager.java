...
public class KeysManager {
    private static final BigInteger plainDefaultKey = new BigInteger("937810634060551071826485204471949219646466658841719067506");
    ...
    public KeysManager() {
        this.defaultSymmetricKey = new SecretKeySpec(KeysManager.plainDefaultKey.toByteArray(),
                Configuration.getInstance().getShareEncryptionAlgorithm());
        try {
            this.signingPublicKey = new PublicKey[1];
            KeyFactory keyFactory = KeyFactory.getInstance("EC");
            for (int i = 0; i < this.signingPublicKey.length; ++i) {
                EncodedKeySpec publicKeySpec = new X509EncodedKeySpec(KeysManager.defaultPublicKey.toByteArray());
                this.signingPublicKey[i] = keyFactory.generatePublic(publicKeySpec);
            }
            EncodedKeySpec privateKeySpec = new PKCS8EncodedKeySpec(KeysManager.defaultPrivateKey.toByteArray());
            this.mySigningPrivateKey = keyFactory.generatePrivate(privateKeySpec);
        }
