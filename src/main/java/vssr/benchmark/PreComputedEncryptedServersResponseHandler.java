        ...
        for (LinkedList<EncryptedConfidentialMessage> msgList : msgs.values()) {
            if (msgList.size() == sameContent) {
                ...
                if (firstMsg.getShares() != null) {
                    ...
                    for (int k = 0; k < numSecrets; ++k) {
                        ...
                        try {
                            ...
                            return new TOMMessage(lastMsg.getSender(), lastMsg.getSession(), lastMsg.getSequence(), lastMsg.getOperationId(), extractedResponse.serialize(), new byte[0], lastMsg.getViewID(), lastMsg.getReqType());
                        }
                    }
                }
                ...
                return new TOMMessage(lastMsg.getSender(), lastMsg.getSession(), lastMsg.getSequence(),
                        lastMsg.getOperationId(), extractedResponse2.serialize(), new byte[0], lastMsg.getViewID(), lastMsg.getReqType());
            }
        }
