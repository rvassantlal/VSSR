...
public class PreComputedPlainServersResponseHandler extends ServersResponseHandler {
    ...
    public TOMMessage extractResponse(TOMMessage[] replies, int sameContent, int lastReceived) {
        ...
        for (LinkedList<ConfidentialMessage> msgList2 : msgs.values()) {
            if (msgList2.size() == sameContent) {
                ConfidentialMessage firstMsg = msgList2.getFirst();
                ...
                if (firstMsg.getShares() != null) {
                    ...
                    for (ConfidentialMessage confidentialMessage : msgList2) {
                    ...
                    for (int k = 0; k < numSecrets; ++k) {
                        ...
                        try {
                            ...
                            return new TOMMessage(lastMsg.getSender(), lastMsg.getSession(), lastMsg.getSequence(),
                                    lastMsg.getOperationId(), extractedResponse.serialize(), new byte[0], lastMsg.getViewID(), lastMsg.getReqType());
                        }
                    }
                }
                ExtractedResponse extractedResponse2 = new ExtractedResponse(plainData, confidentialData);
                TOMMessage lastMsg2 = replies[lastReceived];
                return new TOMMessage(lastMsg2.getSender(), lastMsg2.getSession(), lastMsg2.getSequence(), lastMsg2.getOperationId(), extractedResponse2.serialize(), new byte[0], lastMsg2.getViewID(), lastMsg2.getReqType());
            }
        }
